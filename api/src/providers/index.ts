 ...
 *  guardrails, translators, embeddings, image/music generators, OCR… They pass a
 *  1-token health probe but cannot hold a conversation — "available" yet useless.
 *  We filter them out at discovery so they are never listed, routed to, or probed.
...
abstract class OpenAICompatible implements AIProvider {
  ...
  async generate(req: AIRequest): Promise<AIResponse> {
    // Whole-body timeout: some providers buffer a full completion, which can legitimately
    // take a while — 90s covers that without letting a wedged connection hang forever.
  ...
  async *stream(req: AIRequest): AsyncIterable<AIStreamChunk> {
    // Time-to-first-byte watchdog, cleared the moment headers or the first chunk
    // arrive — a valid slow stream must NOT be killed mid-answer. (A plain
    // AbortSignal.timeout would abort even healthy 45s+ streams.)
